...
var _ = ginkgo.Describe("Volume Expansion Test", func() {
	...
	*/
	...
	/*
		...
		11. Make sure file system has increased
	*/
	ginkgo.It("[csi-supervisor] Verify in Offline volume expansion FileSystemResize "+
		"works when SPS-Service is down", ginkgo.Label(p1, block, wcp, vc70), func() {
		...
		ginkgo.By("File system resize should work though SPS is down")
		...
		gomega.Expect(isFailureFound).To(
			gomega.BeTrue(), "Expected message %v, to occur but did not occur", expectedMsg)
