...
var _ = ginkgo.Describe("[csi-guest] Volume Expansion Test", func() {
	...
	*/
	...
	ginkgo.It("Online volume resize on statically created PVC on guest cluster svcPVC=gcPVC", func() {
		...
		gcPVC, gcPV, pod, _ := createStaticPVandPVCandPODinGuestCluster(client, ctx, namespace, svpvcName, "5Gi", storageclass, v1.PersistentVolumeReclaimDelete)
		defer func() {
	...
	ginkgo.It("Online volume resize on statically created PVC on guest cluster when gcPVC<svcPVC", func() {
		...
		gcPVC, gcPV, pod, _ := createStaticPVandPVCandPODinGuestCluster(client, ctx, namespace, svpvcName, "1Gi", storageclass, v1.PersistentVolumeReclaimDelete)
	...
	ginkgo.It("Online volume resize on statically created PVC on guest cluster when svcPVC<gcPVC", func() {
		...
		gcPVC, gcPV, pod, _ := createStaticPVandPVCandPODinGuestCluster(client, ctx, namespace, svpvcName, "7Gi", storageclass, v1.PersistentVolumeReclaimDelete)
	...
	*/
	...
	//     with PV.
	//  3. Create a pod using the pvc created in step 2 in GC and wait for FS init.
	//  4. Bring CSI-controller pod down in GC.
	//  4. Resize PVC with new size.
	//  5. Check PVC in SVC is in "Resizing" state.
	//  6. Check Size from CNS query is same as what was used in step 5.
	//  7. Bring the CSI-controller pod up in GC.
	//  8. Wait for new size of PVC in GC and compare with SVC PVC size for
	//     equality.
	//  9. Delete POD, PVC, SC

	ginkgo.It("verify Online block volume expansion succeeds when GC CSI pod is down when SVC PVC reaches FilesystemResizePending state and GC CSI comes up", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
...
func createStaticPVandPVCandPODinGuestCluster(client clientset.Interface,
	...
	storageclass *storagev1.StorageClass, pvReclaimPolicy v1.PersistentVolumeReclaimPolicy) (*v1.PersistentVolumeClaim, *v1.PersistentVolume, *v1.Pod, string) {

	gcPVC, gcPV := createStaticPVandPVCinGuestCluster(client, ctx, namespace, svpvcName, size, storageclass, v1.PersistentVolumeReclaimRetain)
	// Create a Pod to use this PVC, and verify volume has been attached.
	ginkgo.By("Creating pod to attach PV to the node")
...
func createStaticPVandPVCinGuestCluster(client clientset.Interface,
	ctx context.Context, namespace string, svpvcName string, size string,
	storageclass *storagev1.StorageClass, pvReclaimPolicy v1.PersistentVolumeReclaimPolicy) (*v1.PersistentVolumeClaim, *v1.PersistentVolume) {
